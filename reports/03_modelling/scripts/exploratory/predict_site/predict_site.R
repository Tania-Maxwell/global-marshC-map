...
site_predictors0 <- raster::stack("reports/05_modelling/data/export_site_all_layers_30m.tif")
site_predictors0
names(site_predictors0)
...
df0 <- read.csv("reports/04_data_process/data/data_cleaned_SOMconverted.csv")
...
summary(df1_intersection$SOCD_g_cm3)
...
raster_plot <- function(x, df) {
  ...
  png(filename = paste("reports/05_modelling/figures/site_predictors/", 
                       names(df[[x]]), ".png", sep = ""), 
      #res = 120,
...
NNDM_cv <- nndm(samplepoints, modeldomain = mask)
print(NNDM_cv)
...
### Import model run previously to compare

model_nndm_export = readRDS("reports/05_modelling/data/model_nndm_site_v_1_4.rds")
model_nndm_export
...
names(site_predictors_forpred_0m)                                    
...
str(AOA$parameters$train)
str(AOA$parameters$weight)

AOA$parameters$threshold # everything larger than this threshold will be outside of aoa

plot(AOA$DI)
plot(AOA$AOA)# just have inside (1) or outside (0) the AOA


compare <- stack(prediction_0_100cm_t_ha, AOA$AOA)
plot(compare)

png(filename = paste("reports/05_modelling/figures/site_SOC_t_ha/", 
                     "site_SOC_t_ha_AOA", ".png", sep = ""), 
    #res = 120,
    width = 888, height = 335.2)
