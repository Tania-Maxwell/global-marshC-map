...
model_spatial # higher error estimates than nndm model ( see RMSE)
...
model_ffs <- ffs(trainDat[,names(predictors)], # feed all potential predictors
                 trainDat$response,
                 method="rf",
                 ntree=100,
                 importance=TRUE,
...
model_nndm <- train(trainDat[,model_ffs$selectedvars], # train only with selected variables
                 trainDat$response,
                 method="rf",
                 ntree=100,
                 importance=TRUE,
...
#tutorial in package on how to parallelize this approach 
...
library(mlr3extralearners) # not on CRAN
...
# the line below doesn't work, likely because mlr3extralearners doesn't exist anymore! 
...
### Estimate AOA
#can't feed in the model itself (like in caret above), but you need to feed in the training data 
AOA <- aoa(predictors,
