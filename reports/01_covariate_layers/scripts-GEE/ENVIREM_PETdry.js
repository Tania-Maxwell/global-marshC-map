// Script to export mean monthly Potential EvapoTranspiration (PET) of driest quarter mm month–1
//06.09.22
...
var covariateName = 'PETdry'; // <<-- CHANGE FOR EACH EXPORT for filenaming
...
/// PET dry 
// PETDriestQuarter mean monthly PET of driest quarter mm month–1

var PETdryoriginal = ee.Image('users/tlgm2/layer_imports/current_30arcsec_PETDriestQuarter');

var layer_toreduce = PETdryoriginal ;
...
// //using kernel number so that it extends over 1km = 1000m
// // here, pixel size in 927.67m sp 1000/927.67 = 1 pixels
// var PETdryreduced1 = PETdryoriginal
//     .reduceNeighborhood({
//       reducer: ee.Reducer.mean(), // mean of values around 
//       kernel: ee.Kernel.circle(1, 'pixels'), // circle recommended over square (Daniele)
//       skipMasked: false //we want to include the masked values, this is where we want to predict
...
// //need to extend a second time, also only by 1 pixel to cover the whole area
// var PETdryreduced2 = PETdryreduced1
//     .reduceNeighborhood({
//       reducer: ee.Reducer.mean(), // mean of values around 
//       kernel: ee.Kernel.circle(1, 'pixels'), // circle recommended over square (Daniele)
//       skipMasked: false //we want to include the masked values, this is where we want to predict
...
// //  this is the layer to export
// var PETdry = PETdryoriginal
//     .unmask(PETdryreduced2) //unmask values using the values recalculated above
//     .updateMask(dataMask) //masking with the topyBathyEcoMask
// //    .clip(site)
// //    .int() // don't need decimals really
//     .rename(covariateName) ;
...
var PETdry_exported = ee.Image('users/tlgm2/covariate_global/PETdry_v0_1');
...
//Map.addLayer(PETdry, PETdryParams, "Reduced Mean PET of driest quarter (mm month-1)");
//Map.addLayer(PETdryoriginal, PETdryParams, "Original Mean PET of driest quarter (mm month-1)");
Map.addLayer(PETdry_exported, PETdryParams, "Exported Mean PET of driest quarter (mm month-1)");
...
var layer_for_boolean0 = PETdry_exported.abs().add(1).unmask(0);
...
var vars = {
  ...
  // endDate:endDate,
  sourceData: 'ENVIREM',
  sourceLink:'https://deepblue.lib.umich.edu/data/concern/data_sets/gt54kn05f',
  covariateName: covariateName,
  generationScript: 'https://code.earthengine.google.com/cd27eb9fe270a18fa338bbceb7c3aeeb',
  ...
  title: 'Mean monthly Potential EvapoTranspiration (PET) of driest quarter (mm month–1) extended to 50 km [Maxwell Global Tidal Marsh Carbon Covariate Layers v0.1]',
  description: 'Covariate layers used for testing in the tidal marsh carbon project',
  citation: 'Title, P. O., & Bemmels, J. B. (2018). ENVIREM: an expanded set of bioclimatic and topographic variables increases flexibility and improves performance of ecological niche modeling. Ecography, 41(2), 291-307.',
...
Export.image.toAsset({
  image: PETdry.set(vars), //include variables set above
  description: 'export_'
    .concat(covariateName),
    // .concat('_')
    // .concat(startDate.slice(0,4))
